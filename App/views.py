...
class expenseAction:
    ...
    def update_expense(request, pk):
        new_expense = Expense.objects.filter(id=pk)
        if request.method == 'POST':
             new_day= request.POST.get('day')
             new_date = request.POST['date']
             new_category = request.POST['category']
...
class salesAction:
    ...
    def update_sales(request):
        #sale_selected = Sales.objects.filter(id=pk)
        if request.method== 'POST':
            ...
            new_sale = Sales(name=sale_name,day=sale_day,quantity=sale_quantity,amount=sale_amount,description=sale_description,image=image,date=sale_date,category=sale_category)
            new_sale.save()
            if new_sale.save():
        ...
        return render(request, 'sales_update.html')
